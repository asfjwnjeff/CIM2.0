...
export default function EditShipperConsigneePage() {
  ...
  const [formData, setFormData] = useState({
    name: "", code: "", type: "both",
    unifiedSocialCreditCode: "", contactPerson: "", phone: "", email: "", remark: "",
  });

  useEffect(() => {
    fetch(`/api/shipper-consignees?serviceEntityId=${serviceEntityId}`)
      .then((r) => r.json())
      .then((data) => {
        if (data.success) {
          const found = data.data.find((s: { id: string }) => s.id === scId);
          if (found) {
            setFormData({ name: found.name || "", code: found.code || "", type: found.type || "both",
              unifiedSocialCreditCode: found.unifiedSocialCreditCode || "", contactPerson: found.contactPerson || "",
              phone: found.phone || "", email: found.email || "", remark: found.remark || "" });
          } else { toast.error("收发货方不存在"); router.push(`/entities/service/${serviceEntityId}`); }
        }
      })
      .catch(() => toast.error("网络错误"))
      .finally(() => setFetching(false));
  }, [serviceEntityId, scId, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const resp = await fetch("/api/shipper-consignees", {
        ...
        body: JSON.stringify({ id: scId, name: formData.name.trim(), type: formData.type, unifiedSocialCreditCode: formData.unifiedSocialCreditCode || undefined, contactPerson: formData.contactPerson || undefined, phone: formData.phone || undefined, email: formData.email || undefined, remark: formData.remark || undefined }),
      });
      const data = await resp.json();
      if (data.success) { toast.success("收发货方更新成功"); router.push(`/entities/service/${serviceEntityId}`); }
    ...
    <div className="max-w-[1440px] mx-auto space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <h1 className="text-2xl font-bold text-[#0A0A0A]">编辑收发货方 — {formData.name}</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-2xl border border-[#EBEBEB] shadow-[0_2px_8px_rgba(0,0,0,0.06)] p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2"><Label className="block text-sm font-semibold text-[#0A0A0A]"><span className="text-red-500">*</span> 名称</Label><Input name="name" value={formData.name} onChange={handleInputChange} className="border-[#D5D5D5] focus:border-[#2D3BFF]" /></div>
            ...
            <div className="space-y-2"><Label className="block text-sm font-semibold text-[#0A0A0A]">类型</Label><Select value={formData.type} onValueChange={(v) => setFormData((p) => ({ ...p, type: v }))}><SelectTrigger className="border-[#D5D5D5]"><SelectValue /></SelectTrigger><SelectContent className="bg-white"><SelectItem value="shipper">发货方</SelectItem><SelectItem value="consignee">收货方</SelectItem><SelectItem value="both">收发货方（兼具）</SelectItem></SelectContent></Select></div>
            <div className="space-y-2"><Label className="block text-sm font-semibold text-[#0A0A0A]">统一社会信用代码</Label><Input name="unifiedSocialCreditCode" value={formData.unifiedSocialCreditCode} onChange={handleInputChange} className="border-[#D5D5D5] focus:border-[#2D3BFF]" /></div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-[#EBEBEB] shadow-[0_2px_8px_rgba(0,0,0,0.06)] p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2 col-span-2"><Label className="block text-sm font-semibold text-[#0A0A0A]">邮箱</Label><Input name="email" type="email" value={formData.email} onChange={handleInputChange} className="border-[#D5D5D5] focus:border-[#2D3BFF]" /></div>
          </div>
        </div>
