...
import { MOCK_USERS, PROGRESS_STATUS_LABELS, RELATION_LABELS, INDUSTRY_CHAIN_LEVEL_LABELS } from '@/lib/sample-data';
...
function buildFormData(customer: ReturnType<typeof useApp>['customers'][number]): EditFormData {
  return {
    ...
    serviceProducts: customer.basicInfo?.serviceProducts || [],
    ...
    customerSystemCode: customer.basicInfo?.customerSystemCode || '',
...
export default function EditCustomerPage() {
  ...
  const handleAdvance = useCallback((status: ProgressStatus) => {
    ...
    setIsDirty(true);
    addLog({ action: 'advance', operator: '系统管理员', targetType: 'customer', targetId: customer.id, targetName: customer.name, details: `跟进进度变更为：${PROGRESS_STATUS_LABELS[status]}` });
  ...
  const handleRollback = useCallback((status: ProgressStatus) => {
    ...
    setForm((prev) => (prev ? { ...prev, progressStatus: status } : prev));
    setIsDirty(true);
    addLog({ action: 'rollback', operator: '系统管理员', targetType: 'customer', targetId: customer.id, targetName: customer.name, details: `跟进进度回退为：${PROGRESS_STATUS_LABELS[status]}` });
  }, [customer, updateCustomerProgress, addLog]);
              ...
              <div className="bg-white rounded-xl border border-[#EBEBEB] p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <SearchableMultiSelect values={form.serviceProducts} onChange={(v) => updateField('serviceProducts', v)} options={SERVICE_PRODUCT_OPTIONS} placeholder="选择服务产品..." searchPlaceholder="搜索..." emptyText="未找到" />
                  </div>
                  ...
                  <div><label className={FIELD_STYLES.label}>客户系统代码</label><input type="text" className={`${FIELD_STYLES.input} bg-[#F5F5F5] text-[#999999] cursor-not-allowed`} value={form.customerSystemCode} readOnly placeholder="风控审批通过后自动生成" /></div>
