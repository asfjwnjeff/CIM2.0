      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold text-[#0A0A0A]">结算主体管理</h1>
            <p className="text-[#5A5A5A] text-[13px] mt-0.5">管理所有结算主体信息，包括基本信息和银行账户数据</p>
          </div>
          <button
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#EBEBEB] bg-[#FAFAFA] h-[36px]">
                  ...
                  <th className="px-3 py-3 text-left text-[11px] font-semibold uppercase text-[#5A5A5A]">开户银行</th>
                  <th className="px-3 py-3 text-left text-[11px] font-semibold uppercase text-[#5A5A5A]">银行账号</th>
                  <th className="px-3 py-3 text-left text-[11px] font-semibold uppercase text-[#5A5A5A]">币种</th>
              ...
              <tbody>
                {filteredEntities.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="text-center text-[#999999] py-12 text-sm">暂无结算主体数据</td>
                  </tr>
                ) : (
                  filteredEntities.map((entity) => (
                    <tr key={entity.id} className="border-b border-[#EBEBEB] h-[44px] hover:bg-[#F5F5F5] transition-colors">
                      ...
                      <td className="px-3 py-3 text-[13px] text-[#0A0A0A]">{entity.bankName || '-'}</td>
                      <td className="px-3 py-3 text-[13px] text-[#5A5A5A] font-mono text-xs">{entity.bankAccount || '-'}</td>
                      <td className="px-3 py-3 text-[13px] text-[#5A5A5A]">{entity.currency || '-'}</td>
